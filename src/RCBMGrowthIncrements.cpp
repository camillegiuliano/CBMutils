...
Rcpp::List ComputeGrowthCoordinateMatrices(PoolNames& pn, Rcpp::NumericMatrix& growthIncrements) {
...
Rcpp::List ComputeOverMatureDeclineCoordinateMatrices(PoolNames& pn, Rcpp::NumericMatrix& decline) {
...
SpinupState UpdateSpinupState(SpinupState current, int finalAge, int stepNum,
  ...
  //Rprintf("Current spinup state: %d\n", current); // debugging
  switch (current) {
  ...
  return Disabled; // needs to return object outside of case
}
