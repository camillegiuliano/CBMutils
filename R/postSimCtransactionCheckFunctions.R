## This function needs the checkProportionalMatrices.R function in this package.
...
## Overview: post simulations this function takes the last two years of
## simulations, and verifies that the c transfers are as expected. It randomly
## selects three pixelGroups, calculated the expected transactions from time
## end(sim) and time end(sim)-1, and compares them to the last year of
## simulations.
...
# TODO there are still a bunch of checks that need to be changed to assertions
# (if statements?)

checkTransactions <- function(sim){
  ### 1. Sim list and packages ################################
  # TODO do we load packages this way in a function? or do they have to be
  ...
  colnames(theseOpMatrices) <- stringr::str_replace_all(colnames(theseOpMatrices), c(" " = ""))
  ...
  pgIN <- melt(inCcheck, measure.vars = InputColNum:ProductsColNum,
  ...
  cStartLong <- melt(cTransaction, measure.vars =  5:13,
  ...
  if(sum(theseOpMatrices$disturbance) > 0){
    ...
    if(dim(newC[,unique(col), by = "pixelGroup"])[1] !=
      ...
      for(i in 1:length(unique(newc$pixelGroup))){
        ...
        add0rows <- as.data.table(cbind(pixelGroup = rep(pixelGroup[i], length(rowsNeeded)),col = rowsNeeded,
    ...
    newC[order(pixelGroup,row),] == checkMats[pixelGroup %in% theseOpMatrices[disturbance != 0,pixelGroup] & order(pixelGroup,row),]
  } else distCmats <- inCmats
  ########################## disturbances done
  ...
  # get the long form of the c pools at the start with "rows" column matching the
  # pool names
  pgOUT <- melt(outCcheck, measure.vars = InputColNum:ProductsColNum,
                id.vars = c("pixelGroup"))
