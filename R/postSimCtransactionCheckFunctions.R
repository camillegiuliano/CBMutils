...
#'
#' All arguments come from a `simList` object of the same name, except timeSim (which comes from time(sim))
#' and outputsSim (which comes from outputs(sim)).
#'
#' @param cbmPools from sim$cbmPools
#' @param pixelKeep from sim$pixelKeep
#' @param opMatrixCBM from sim$opMatrixCBM
#' @param allProcesses from sim$allProcesses
#' @param pixelGroupC from sim$pixelGroupC
#' @param pooldef from sim$pooldef
#' @param timeSim from time(sim)
#' @param outputsSim from outputs(sim)
...
checkTransactions <- function(cbmPools, pixelKeep, opMatrixCBM, allProcesses,
                              pixelGroupC, pooldef, timeSim, outputsSim, n = 3) {
  ### 1. Sim list and packages ################################
  # pick the two last years of the simulation

    allOuts <- as.data.table(outputsSim)
    lastYrPools <- allOuts[saveTime == timeSim - 1 & objectName == "cbmPools", file]
    poolsIn <- readRDS(file = lastYrPools)
    #poolsIn <- cbmPools[simYear == timeSim - 1, ]
    poolsOut <- cbmPools[simYear == timeSim, ]
    ...
    # hwGc <- simDollargcMeta[forest_type_id == 3, growth_curve_component_id]
    # hwPg <- pixelGroupC[growth_curve_component_id %in% hwGc,pixelGroup]
    ...
    if (dim(inCcheck)[1] != n) {
      pKeep <- unique(pixelKeep[, (dim(pixelKeep)[2] - 1):(dim(pixelKeep)[2])])
    ...
    colnames(theseOpMatrices) <- gsub(pattern = " ", replacement = "", colnames(theseOpMatrices))
    ...
    for (i in setdiff(names(processes), setdiff(names(processes), rmNames))) {
      pgs <- rep(1:length(pixelGroupC$pixelGroup), lengths(processes[[i]])/3)
    ...
    poolsToRows <- as.data.frame(cbind(pools = pooldef, row = c(1:26))) ## sim$
    ...
    if (sum(theseOpMatrices$disturbance) > 0) {
      ...
      if (dim(newC[, unique(col), by = "pixelGroup"])[1] !=
        length(pooldef) * length(unique(newC$pixelGroup))) {
        ...
        for (i in 1:length(unique(newc$pixelGroup))) {
          ...
          rowsNeeded <- which(!(c(1:length(pooldef)) %in% newc$V1))
      ...
      if (dim(newC)[1] != (length(pooldef) * length(unique(newC$pixelGroup)))) {
